...
exports.updateProjector = async (req, res) => {
  try {
    ...
    console.log('📝 Updating projector with:', updatedData);
...
// Get projector filter options
exports.getProjectorFilters = async (req, res) => {
  try {
    // Get distinct statuses
    const statuses = await Projector.distinct('status');

    // Get distinct types
    const types = await Projector.distinct('type');

    // Get distinct manufacturers
    const manufacturers = await Projector.distinct('manufacturer');
    ...
    const departmentResults = await Projector.aggregate(departmentPipeline);
    const departments = departmentResults.map(result => result._id).filter(dept => dept);

    // Get year range
    const yearStats = await Projector.aggregate([
      {
        $group: {
          _id: null,
    ...
    console.error('Lỗi getProjectorFilters:', error);
