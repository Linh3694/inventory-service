...
exports.uploadHandoverReport = async (req, res) => {
  try {
    ...
    if (!tool) return res.status(404).json({ message: 'Không tìm thấy thiết bị.' });
    let currentAssignment = tool.assignmentHistory.find((h) => h.user && h.user.toString() === userId && !h.endDate);
    if (!currentAssignment) {
      tool.assignmentHistory.push({ user: new mongoose.Types.ObjectId(userId), startDate: new Date(), document: originalFileName });
    ...
    tool.status = 'Active';
    await tool.save();
    return res.status(200).json({ message: 'Tải lên biên bản thành công!', tool });
  } catch (error) {
